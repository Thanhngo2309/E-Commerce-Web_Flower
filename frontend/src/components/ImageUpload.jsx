                            ...
                            productData.productImg.map((file, index) => {
                                ...
                                return (
                                    <Card key={index} className='relative group overflow-hidden'>
                                        <CardContent>
                                            <img src={preview} alt="" width={200} height={200} className='w-full h-32 oject-cover rounded-md' />

                                            {/* remove button */}

                                            <button onClick={()=> removeImage(index)} className='absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition'><X sizs={14}/></button>
                                        </CardContent>
                                    </Card>
                                )
                            })
